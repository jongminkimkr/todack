...
@keyframes star-orbit{to{transform:rotate(360deg)}}
    ...
    ${upcoming ? `
      ...
      <a class="card card--tap next-book glare" href="#/booking" data-reveal>
        <span class="next-book__date">
          ...
          <small>${upcoming.slot.split(' ')[0]}</small>
        </span>
...
새로고침 후 콘솔에서 `selfTest()` → `전체 20건 통과`.
...
selfTest()
```
기대: `전체 39건 통과`, 반환값 `true`. **한 건이라도 실패하면 다음 단계로 넘어가지 않는다.**
