...
- **app.html / index.html 이중 파일** — 두 파일은 동기화 유지가 의무. app.html에는 safe-area 관련 코드(85줄 차이)가 추가되어 있다. `selfTest()`는 index.html에만 있음.
...
사용자 입력 → render 함수(DOM 이벤트) → STORE(Moods/Bookings) → DB(localStorage)
                                       → LOGIC 함수(순수, 부수효과 없음)
간편상담: step0(기분) → step1(텍스트) → normalizeText() → pickAdvice()
         → step2(reflect+advice) → step2a(duration) → step2b(focus) → step3(팁카드)
```

## 기능 확장 시 고려할 점

- 새 저장 항목 추가 시 `localStorage.clear()` 대신 개별 키 삭제를 지원해야 한다 (공용 PC 프라이버시 요건).
...
- `RULES` 카테고리 추가 시 `Q2_MAP`도 반드시 함께 확장 (현재 56개 × 3개 선택지).
- `MOODS` 배열 변경 시 `CHAT_MOODS`도 동기화 필요 (`MOOD_BY_KEY`는 자동 반영됨).
